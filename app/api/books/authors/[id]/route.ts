...
import { authorsStore, getBooksByAuthor, initializeBooksData } from '@/lib/data/books/store';
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  const author = await authorsStore.get(id);
  if (!author) {
    return notFound(`Author with ID ${id}`);
  }

  // Include books if requested
  const includeBooks = request.nextUrl.searchParams.get('include_books') === 'true';

  let result: Record<string, unknown> = { ...author };
  if (includeBooks) {
